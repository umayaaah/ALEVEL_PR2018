    ...
    class Paper1Alevel2018CS
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (Choice != "9")
            {
                ...
                if (Choice == "1")
                {
                    PlayGame(AllowedWords, TileDictionary, 1, StartHandSize, MaxHandSize, MaxTilesPlayed, NoOfEndOfTurnTiles);
                }
                else if (Choice == "2")
                {
                    PlayGame(AllowedWords, TileDictionary, 2, 15, MaxHandSize, MaxTilesPlayed, NoOfEndOfTurnTiles);
                }
                else if (Choice == "3")
                {
                    PlayGame(AllowedWords, TileDictionary, 3, 15, MaxHandSize, MaxTilesPlayed, NoOfEndOfTurnTiles);
                }
            }
        }
        ...
        private static void PlayGame(List<string> AllowedWords, Dictionary<char, int> TileDictionary, int RandomStart, int StartHandSize, int MaxHandSize, int MaxTilesPlayed, int NoOfEndOfTurnTiles)
        {
            ...
            else
            {
                ...
                List <int> TilesPlayed = new List<int>();
                try
                {
                    StreamReader filetoRead = new StreamReader(filename);
                    while (!filetoRead.EndOfStream)
                    {
                        int Score = Convert.ToInt32(filetoRead.ReadLine().Split(',')[0]);
                        Scores.Add(Score);

                        string Tile = filetoRead.ReadLine().Split(',')[1];
                        Tiles.Add(Tile);

                        int TilePlayed = Convert.ToInt32(filetoRead.ReadLine().Split(',')[2]);
            ...
            while (PlayerOneTilesPlayed <= MaxTilesPlayed && PlayerTwoTilesPlayed <= MaxTilesPlayed && PlayerOneTiles.Length < MaxHandSize && PlayerTwoTiles.Length < MaxHandSize)
            {
                ...
                Console.Write("Press 9 to save game: ");
                int Choice = Convert.ToInt32(Console.ReadLine());
                if (Choice == 9)
                {
                    Console.Write("Enter file name: ");
                    string FileName = Console.ReadLine();
                    SaveGame(FileName, PlayerOneScore, PlayerTwoScore, PlayerOneTiles,
                        PlayerTwoTiles, PlayerOneTilesPlayed, PlayerTwoTilesPlayed);
                }
            }
        ...
        private static void SaveGame(string FileName, int PlayerOneScore, int PlayerTwoScore, 
            string PlayerOneTiles, string PlayerTwoTiles, int PlayerOneTilesPlayed, int PlayerTwoTilesPlayed)
        {
            StreamWriter writetoFile = new StreamWriter(FileName);
            writetoFile.WriteLine(PlayerOneScore + ", " + PlayerOneTiles + ", " + PlayerOneTilesPlayed);
            writetoFile.WriteLine(PlayerTwoScore + ", " + PlayerTwoTiles + ", " + PlayerTwoTilesPlayed);
            writetoFile.Close();
        ...
        private static void DisplayMenu()
        {
            ...
            Console.WriteLine("3. Load existing game");
