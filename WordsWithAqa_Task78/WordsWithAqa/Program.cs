...
namespace WordsWithAqa
{
    class Paper1Alevel2018CS
    {
        class QueueOfTiles
        {
            ...
            public void Add()
            {
                ...
                if (Rear < MaxSize - 1)
                {
                    ...
                    } while (LastTile == Convert.ToChar(65 + RandNo).ToString());
                    
                    Rear++;
                    Contents[Rear] = Convert.ToChar(65 + RandNo).ToString();
                    LastTile = Contents[Rear];
                }
            }
        ...
        private static string GetChoice()
        {
            ...
            Console.WriteLine("     press 2 to sqap your letters OR ");
        ...
        private static void HaveTurn(string PlayerName, ref string PlayerTiles, ref int PlayerTilesPlayed, ref int PlayerScore, Dictionary<char, int> TileDictionary, ref QueueOfTiles TileQueue, List<string> AllowedWords, int MaxHandSize, int NoOfEndOfTurnTiles)
        {
            ...
            while (!ValidChoice)
            {
                ...
                {
                    PlayerTiles = GetStartingHand(TileQueue, PlayerTiles.Length);
                    Console.WriteLine("New tiles: {0}", PlayerTiles);
                }
                else if (Choice == "4")
                {
                    TileQueue.Show();
