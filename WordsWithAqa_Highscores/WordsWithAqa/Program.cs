...
namespace WordsWithAqa
{
    class Paper1Alevel2018CS
    {
        ...
        private static void DisplayWinner(int PlayerOneScore, int PlayerTwoScore)
        {
            ...
            SaveScore(PlayerOneScore, PlayerTwoScore);
            Console.WriteLine();
        }

        private static void SaveScore(int Score1, int Score2)
        {
            StreamWriter highscoreFile = new StreamWriter("highscores.txt", true);
            highscoreFile.WriteLine(Score1.ToString());
            highscoreFile.WriteLine(Score2.ToString());
            highscoreFile.Close();
        }

        private static void OutputHighScore()
        {
            StreamReader readHiScFile = new StreamReader("highscores.txt");
            List<int> Scores = new List<int>();
            int tempScore = 0;
            while(!readHiScFile.EndOfStream)
            {
                tempScore = Convert.ToInt32(readHiScFile.ReadLine());
