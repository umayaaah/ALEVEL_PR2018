...
namespace WordsWithAqa
{
    class Paper1Alevel2018CS
    {
        class LetterTile
        {
            ...
            public LetterTile(char Letter, Dictionary<Char, int> TileDictionary)
            {
                this.Letter = Letter;
                IsVowel = false;

                int asciiValue = (int)Letter;
                int [] VowelArray = new int []{65, 70, 74, 80, 86};
                if(VowelArray.Contains(asciiValue))
                {
                    IsVowel = true;
                }

                Score = TileDictionary[Letter];
            }
        }
        ...
        static void Main(string[] args)
        {
            ...
            while (Choice != "9")
            {
                ...
                else if (Choice == "3")
                {
                    ...
                    MaxHandSize = Convert.ToInt32(Console.ReadLine());

                    Console.Write("Enter maximum tiles played: ");
                    MaxTilesPlayed = Convert.ToInt32(Console.ReadLine());

                    Console.Write("Default new tiles: ");
                    NoOfEndOfTurnTiles = Convert.ToInt32(Console.ReadLine()); 

                    Console.Write("Enter starting hand: ");
                ...
                else
                {
                    ...
                    if (ValidWord)
                    {
                        ...
                        if (ValidWord)
                        {
                            ...
                            // Outputs number of tiles before asking for new tile choice
                            Console.WriteLine("You have {0} tiles remaining", PlayerTiles.Count());
                    ...
                    if (NewTileChoice != "4")
                    {
                        ...
                        // If they have added more tiles then new number of tiles outputted
                        Console.WriteLine("You have {0} tiles remaining", PlayerTiles.Count());
                    }
                    Console.WriteLine();
