...
namespace WordsWithAqa
{
    class Paper1Alevel2018CS
    {
        class QueueOfTiles
        {
            ...
            public void Add()
            {
                ...
                if (Rear < MaxSize - 1)
                {
                    ...
                    } while (Contents.Contains(Convert.ToChar(65 + RandNo).ToString()));
        ...
        private static void ExchangeTiles(ref string PlayerTiles,
            ref int PlayerScore, ref QueueOfTiles TileQueue)
        {
            string newTiles = "";

            for (int i = 0; i < PlayerTiles.Length; i++)
            {
                newTiles = newTiles + TileQueue.Remove();
                TileQueue.Add();
            }

            Console.WriteLine("Tiles exchanged. New tiles: {0}", newTiles);
            PlayerTiles = newTiles;

            PlayerScore = PlayerScore - 10;
            Console.WriteLine("10 point penalty incurred. New score: {0}");
        }

        private static string ResolveWildCards(string Word, ref string PlayerTiles)
        {
            string newWord = "";
            int wildcardCount = 0;
            ...
            foreach (char c in Word)
            {
                ...
                else if (c == '*' && wildcardCount == 0)
                {
                    Console.WriteLine("Which letter would you like" +
                                        "the wilcard to represent?");
        ...
        private static void HaveTurn(string PlayerName, ref string PlayerTiles, ref int PlayerTilesPlayed, ref int PlayerScore, Dictionary<char, int> TileDictionary, ref QueueOfTiles TileQueue, List<string> AllowedWords, int MaxHandSize, int NoOfEndOfTurnTiles)
        {
            ...
            while (!ValidChoice)
            {
                ...
                else
                {
                    ...
                    {
                        Choice = ResolveWildCards(Choice, ref PlayerTiles);
