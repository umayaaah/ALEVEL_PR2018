...
namespace WordsWithAqa
{
    class Paper1Alevel2018CS
    {
        ...
        private static bool CheckWordBinarySearch(string Word, List<string> AllowedWords)
        {
            bool ValidWord = false;
            int Low = 0;
            int High = AllowedWords.Count;
            int Mid = 0;

            while (!ValidWord && Low <= High)
            {
                Mid = (Low + High) / 2;
                if (AllowedWords[Mid].CompareTo(Word) == 0)
                {
                    ValidWord = true;
                }
        ...
        private static void DisplayTilesInHand(string PlayerTiles, Dictionary<char, int> TileDictionary)
        {
            ...
            {
                Console.Write("{0} ({1}) ", PlayerTiles[i], TileDictionary[PlayerTiles[i]]);
            }
        }
            ...
            while (!ValidChoice)
            {
                ...
                else
                {
                    ...
                    if (Choice.Length == 0)
                    {
                        ValidWord = false;
                        PlayerScore -= 10;
                        Console.WriteLine("Penalty 10 points removed. Your score: {0}", PlayerScore);
                    }
                    else
                    {
                    ...
                    if (ValidWord)
                    {
                        ValidWord = CheckWordBinarySearch(Choice, AllowedWords);
                        if (ValidWord)
                        {
                            Console.WriteLine();
