...
namespace WordsWithAqa
{
    class Paper1Alevel2018CS
    {
        ...
        private static void FillHandWithTiles(ref QueueOfTiles TileQueue, ref string PlayerTiles, int MaxHandSize, bool Check, Dictionary<char, int> TileDictionary)
        {
            if(Check)
            {
                string TempTiles = PlayerTiles;
                int HandValue = 0;
                while (TempTiles.Length <= MaxHandSize)
                {
                    TempTiles = TempTiles + TileQueue.Remove();
                    TileQueue.Add();
                }
                foreach(char c in TempTiles)
                {
                    HandValue += TileDictionary[c];
                }
                Console.WriteLine("Your hand value would be: " + HandValue);
            }
            else
            {
        ...
        private static string GetChoice()
        {
            ...
            while (!ValidChoice)
            {
                ...
                Console.WriteLine("     press 2 to skip turn OR");
                ...
                if (MenuChoices.Contains(Choice))
                {
                    return Choice;
                }
                ValidChoice = true;
        ...
        private static string ShuffleTiles(string PlayerTiles)
        {
            char[] ShuffledTiles = PlayerTiles.ToCharArray();
            char tempTile;
            Random rnd = new Random();
            int randIndex;
            for (int i = 0; i < PlayerTiles.Length; i++)
            {
                tempTile = ShuffledTiles[i];
                randIndex = rnd.Next(0, PlayerTiles.Length);

                ShuffledTiles[i] = ShuffledTiles[randIndex];
                ShuffledTiles[randIndex] = tempTile;
            }
                    
            return new string(ShuffledTiles);
        }

        private static string OrderTiles(string PlayerTiles)
        {
            char[] OrderedTile = PlayerTiles.ToCharArray();
            //Array.Sort(OrderedTile);
            char tempTile;
            for(int i =0; i <PlayerTiles.Length; i++)
            {
                for(int j = 0; j < PlayerTiles.Length-1; j++)
                {
                    tempTile = OrderedTile[j];
                    if(OrderedTile[j] > OrderedTile[j+1])
                    {
                        OrderedTile[j] = OrderedTile[j + 1];
                        OrderedTile[j + 1] = tempTile;
                    }
                }
            }
            return new string(OrderedTile);
        }
        ...
        private static void HaveTurn(string PlayerName, ref string PlayerTiles, ref int PlayerTilesPlayed, ref int PlayerScore, Dictionary<char, int> TileDictionary, ref QueueOfTiles TileQueue, List<string> AllowedWords, int MaxHandSize, int NoOfEndOfTurnTiles)
        {
            ...
            while (!ValidChoice)
            {
                ...
                {
                    PlayerTiles = ShuffleTiles(PlayerTiles);
                    DisplayTilesInHand(PlayerTiles);

                    PlayerTiles = OrderTiles(PlayerTiles);
                    DisplayTilesInHand(PlayerTiles);
                }
                else if (Choice == "4")
                {
                ...
                {
                    ValidChoice = true;
                    FillHandWithTiles(ref TileQueue, ref PlayerTiles, MaxHandSize, false, TileDictionary);
                }
                else
                {
                    ValidChoice = true;
