    ...
    class Paper1Alevel2018CS
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (Choice != "9")
            {
                ...
                if (Choice == "1")
                {
                    PlayGame(AllowedWords, TileDictionary, true, StartHandSize, MaxHandSize, MaxTilesPlayed, NoOfEndOfTurnTiles, BannedWords);
                }
                else if (Choice == "2")
                {
                    PlayGame(AllowedWords, TileDictionary, false, 15, MaxHandSize, MaxTilesPlayed, NoOfEndOfTurnTiles, BannedWords);
                }
                else if (Choice == "3")
                {
                    ...
                    do
                    {
                        Console.Write("Enter banned word: ");
                        UserChoice = Console.ReadLine().ToUpper().ToString();
                        if (UserChoice != "0")
                        {
                            BannedWords.Add(UserChoice);
                        }
                    } while (UserChoice != "0");
                }
            }
        ...
        private static void HaveTurn(string PlayerName, ref string PlayerTiles, ref int PlayerTilesPlayed, ref int PlayerScore, Dictionary<char, int> TileDictionary, ref QueueOfTiles TileQueue, List<string> AllowedWords, int MaxHandSize, int NoOfEndOfTurnTiles, List<string> BannedWords)
        {
            Console.WriteLine();
            ...
            while (!ValidChoice)
            {
                ...
                else
                {
                    ...
                    else if(BannedWords.Contains(Choice))
                    {
                        Console.WriteLine("That word is not allowed! You lose 5 points");
        ...
        private static void PlayGame(List<string> AllowedWords, Dictionary<char, int> TileDictionary, bool RandomStart, int StartHandSize, int MaxHandSize, int MaxTilesPlayed, int NoOfEndOfTurnTiles, List<string> BannedWords)
        {
            ...
            {
                HaveTurn("Player One", ref PlayerOneTiles, ref PlayerOneTilesPlayed, ref PlayerOneScore, TileDictionary, ref TileQueue, AllowedWords, MaxHandSize, NoOfEndOfTurnTiles, BannedWords);
                Console.WriteLine();
                Console.WriteLine("Press Enter to continue");
                Console.ReadLine();
                Console.WriteLine();
                HaveTurn("Player Two", ref PlayerTwoTiles, ref PlayerTwoTilesPlayed, ref PlayerTwoScore, TileDictionary, ref TileQueue, AllowedWords, MaxHandSize, NoOfEndOfTurnTiles, BannedWords);
            }
        ...
        private static void DisplayMenu()
        {
            ...
            Console.WriteLine("3. Add banned words");
