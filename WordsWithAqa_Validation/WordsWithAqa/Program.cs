    ...
    class Paper1Alevel2018CS
    {
        class QueueOfTiles
        {
            ...
            public void Add()
            {
                ...
                if (Rear < MaxSize - 1)
                {
                    RandNo = Rnd.Next(0, 101);
                    if (RandNo < 40) // This is the 40% chance of getting below 40
                    {
                        RandNo = Value1[Rnd.Next(0, Value1.Length)];
                    }
                    else if (RandNo < 70) // If its not less than 40, there is a 30% chance of being less than 70
                    {
                        RandNo = Value2[Rnd.Next(0, Value2.Length)];
                    }
                    else if (RandNo < 90) // If its not less than 70, there is a 20% chance of being less than 90
                    {
                        RandNo = Value3[Rnd.Next(0, Value3.Length)];
                    }
                    else // If its not less than 90, there is a 10% chance it is less than 100
                    {
                        RandNo = Value5[Rnd.Next(0, Value5.Length)];
                    }
        ...
        private static string GetChoice()
        {
            ...
            while(!validInput) // Repeats until validInput is true
            {
                Console.WriteLine();
                ...
                // Check if they've entered a valid menu option
                string[] MenuOptions = { "1", "4", "7", "0" };
                if (MenuOptions.Contains(Choice))
                {
                    return Choice;
                }

                // Check if they've entered a word, with no numbers
                validInput = true;
                foreach (char c in Choice) // Loops through each character in the word
                {
                    int asciiCode = (int)c;
                    if (asciiCode < 65 || asciiCode > 90) // If they've not entered a letter
                    {
                        validInput = false; // This will make them re-enter word
                    }
                }
            }
