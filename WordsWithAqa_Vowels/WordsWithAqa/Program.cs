...
namespace WordsWithAqa
{
    class Paper1Alevel2018CS
    {
        class QueueOfTiles
        {
            ...
            // Changes made here to give 50/50 distribution of vowels & consonants
            public void Add()
            {
                int RandNo = 0;
                ...
                if (Rear < MaxSize - 1)
                {
                    if (Rnd.Next(0,2) == 0) // 50% chance of being a vowel
                    {
                        RandNo = Vowels[Rnd.Next(0, 5)];
                    }
                    else // 50% chance of being a consonant
                    {
                        do
                        {
                            RandNo = Rnd.Next(0, 26);
                        } while (Vowels.Contains(RandNo));
                    }
                    Rear++;
